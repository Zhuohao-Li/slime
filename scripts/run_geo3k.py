...
def execute(args: ScriptArgs):
    ...
    rollout_args = (
        ...
        "--rm-type math "
        "--num-rollout 3000 "
        f"--rollout-batch-size {1 if args.mode == 'debug_minimal' else 32} "
        f"--n-samples-per-prompt {1 if args.mode == 'debug_minimal' else 8} "
        f"--rollout-max-response-len {100 if args.mode == 'debug_minimal' else 8192} "
        "--rollout-temperature 0.8 "
        f"--global-batch-size {1 if args.mode == 'debug_minimal' else 256} "
        "--balance-data "
    )

    if args.dynamic_sampling and (args.mode == "normal"):
        rollout_args += (
    ...
    if (args.mode == "normal") and args.enable_eval:
        eval_args = (
    ...
    match args.train_backend:
        case "fsdp":
            ...
            sglang_args += "--sglang-mem-fraction-static 0.6 "
